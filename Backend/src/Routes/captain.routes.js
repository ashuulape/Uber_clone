...
const registervalidation = [
    ...
    body('vehicale.plate').isLength({ min: 3 }).withMessage('Plate must be at least 3 characters long'),
    body('vehicale.capcity').isInt({ min: 1 }).withMessage('Capacity must be at least 1'),
    body('vehicale.vehicaleType').isIn(['car', 'auto', 'bike']).withMessage('Invalid vehicle type')
]
